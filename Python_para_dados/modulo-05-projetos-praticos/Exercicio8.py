...
media_especies = {area: sum(especies) / len(especies) for area, especies in especies_por_area.items()}
area_maior_diversidade = max(media_especies, key=media_especies.get)

print(f'''Média de espécies por área: {media_especies}
Área com a maior diversidade biológica: {area_maior_diversidade} com média de {media_especies[area_maior_diversidade]:.2f} espécies.''')